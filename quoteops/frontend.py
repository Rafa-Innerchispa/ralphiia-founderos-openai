...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .commercial-grid { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:7px; margin:8px 0; }
    .commercial-grid input, .commercial-grid select { width:100%; min-width:0; padding:9px; border:1px solid var(--line); border-radius:10px; background:white; }
...
<body>
  ...
  <script>
    ...
    Object.assign(I18N.es, {
      ...
      ,commercialSettings:'Condiciones comerciales', customerProfile:'Perfil del cliente', supplierProfile:'Perfil del proveedor', partyId:'ID de parte', partyName:'Nombre de parte', partyType:'Tipo', category:'Categoría', paymentMode:'Modo de pago', creditDays:'Días de crédito', creditStatus:'Estado de crédito', creditLimit:'Límite de crédito', paymentRisk:'Comportamiento o riesgo', preferenceQuality:'Preferencia o calidad', confirmationSource:'Fuente de confirmación', confirmedDate:'Fecha confirmada', notes:'Notas', saveProfile:'Guardar perfil', supplierComparison:'Comparación de proveedores', cheapest:'Menor costo', confirmedCredit:'Crédito confirmado', recommendation:'Recomendación', premium:'Prima/delta', eligibility:'Elegibilidad', missingFacts:'Datos faltantes', policyThreshold:'Umbral de política', creditPremiumPct:'Prima máxima de crédito (%)', status_current_confirmed:'Confirmado vigente', status_historical_observed:'Histórico observado', status_unverified:'Sin verificar', status_unavailable:'No disponible'
    });
    Object.assign(I18N.en, {
      ...
      ,commercialSettings:'Commercial settings', customerProfile:'Customer profile', supplierProfile:'Supplier profile', partyId:'Party ID', partyName:'Party name', partyType:'Type', category:'Category', paymentMode:'Payment mode', creditDays:'Credit days', creditStatus:'Credit status', creditLimit:'Credit limit', paymentRisk:'Payment behavior or risk', preferenceQuality:'Preference or quality', confirmationSource:'Confirmation source', confirmedDate:'Confirmed date', notes:'Notes', saveProfile:'Save profile', supplierComparison:'Supplier comparison', cheapest:'Lowest cost', confirmedCredit:'Confirmed credit', recommendation:'Recommendation', premium:'Premium/delta', eligibility:'Eligibility', missingFacts:'Missing facts', policyThreshold:'Policy threshold', creditPremiumPct:'Maximum credit premium (%)', status_current_confirmed:'Current confirmed', status_historical_observed:'Historical observed', status_unverified:'Unverified', status_unavailable:'Unavailable'
    });
    const WORK_STATUS = {
    ...
    function renderDossier(data) {
      ...
      const profiles = values(data.commercial_profiles);
      const profileEditor = (role) => { const p = profiles.find(x => x.party_role === role) || {terms:{}}; const terms = p.terms || {}; return `<div class="section"><h3>${t(role === 'customer' ? 'customerProfile' : 'supplierProfile')}</h3><div class="commercial-grid" data-profile-role="${role}"><input data-field="party_id" value="${escapeAttr(p.party_id || '')}" placeholder="${t('partyId')}"/><input data-field="party_name" value="${escapeAttr(p.party_name || '')}" placeholder="${t('partyName')}"/><input data-field="party_type" value="${escapeAttr(p.party_type || '')}" placeholder="${t('partyType')}"/><input data-field="category" value="${escapeAttr(p.category || '')}" placeholder="${t('category')}"/><input data-field="payment_mode" value="${escapeAttr(terms.payment_mode || '')}" placeholder="${t('paymentMode')}"/><input data-field="credit_days" type="number" min="0" value="${escapeAttr(terms.credit_days ?? '')}" placeholder="${t('creditDays')}"/><select data-field="credit_status">${['current_confirmed','historical_observed','unverified','unavailable'].map(x => `<option value="${x}" ${terms.credit_status === x ? 'selected':''}>${t('status_'+x)}</option>`).join('')}</select><input data-field="credit_limit" type="number" min="0" value="${escapeAttr(terms.credit_limit ?? '')}" placeholder="${t('creditLimit')}"/><input data-field="payment_behavior_or_risk" value="${escapeAttr(terms.payment_behavior_or_risk || '')}" placeholder="${t('paymentRisk')}"/><input data-field="preference_or_quality" value="${escapeAttr(terms.preference_or_quality || '')}" placeholder="${t('preferenceQuality')}"/><input data-field="confirmation_source" value="${escapeAttr(terms.confirmation_source || '')}" placeholder="${t('confirmationSource')}"/><input data-field="confirmed_date" value="${escapeAttr(terms.confirmed_date || '')}" placeholder="${t('confirmedDate')}"/><input data-field="notes" value="${escapeAttr(terms.notes || '')}" placeholder="${t('notes')}"/>${role === 'supplier' ? `<input data-field="credit_premium_pct" type="number" min="0" max="100" value="${escapeAttr(data.sourcing_policy_max_credit_premium_pct ?? 5)}" placeholder="${t('creditPremiumPct')}"/>` : ''}</div><button class="action secondary save-profile" data-profile-role="${role}">${t('saveProfile')}</button></div>`; };
      const comparisons = values(data.sourcing_recommendations).map(x => `${x.canonical_item_id} · ${t('cheapest')}: ${x.lowest_cost_offer?.supplier_name || t('noData')} · ${t('confirmedCredit')}: ${x.best_confirmed_credit_offer?.supplier_name || t('noData')} · ${t('recommendation')}: ${x.recommended_offer?.supplier_name || t('noData')} · ${t('premium')}: ${x.percentage_delta ?? t('noData')}% · ${t('policyThreshold')}: ${x.policy_max_credit_premium_pct}% · ${t('missingFacts')}: ${(x.recommended_offer?.unknown_facts || []).join(', ') || t('noData')}`);
      let html = '';
      html += listSection(t('workItem'), workItem);
      html += listSection(t('project'), projectItems);
      html += listSection(t('customer'), customerItems);
      html += `<div class="section"><h3>${t('commercialSettings')}</h3>${profileEditor('customer')}${profileEditor('supplier')}</div>`;
      ...
      html += listSection(t('supplierComparison'), comparisons);
      ...
      document.querySelectorAll('.save-profile').forEach((button) => button.addEventListener('click', () => saveCommercialProfile(button.dataset.profileRole)));
    ...
    async function saveCommercialProfile(role) { if (!missionId) { showToast(t('missionFirst')); return; } const root = document.querySelector(`[data-profile-role="${role}"]`); const value = (field) => root.querySelector(`[data-field="${field}"]`)?.value.trim() || ''; const days = value('credit_days'); const limit = value('credit_limit'); const policy = value('credit_premium_pct'); const profile = {party_role:role, party_id:value('party_id'), party_name:value('party_name'), party_type:value('party_type'), category:value('category'), terms:{payment_mode:value('payment_mode'), credit_days:days ? Number(days) : null, credit_status:value('credit_status'), credit_limit:limit ? Number(limit) : null, payment_behavior_or_risk:value('payment_behavior_or_risk'), preference_or_quality:value('preference_or_quality'), confirmation_source:value('confirmation_source'), confirmed_date:value('confirmed_date'), notes:value('notes')}}; try { const result = await fetchJson(`/api/conversation/missions/${encodeURIComponent(missionId)}/commercial-profiles`, {method:'PUT', headers:{'content-type':'application/json'}, body:JSON.stringify({idempotency_key:newKey(), language, profile, max_credit_premium_pct:policy ? Number(policy) : null})}); renderDossier(result.dossier); showToast(t('saved')); } catch(error) { showToast(error.message); } }
