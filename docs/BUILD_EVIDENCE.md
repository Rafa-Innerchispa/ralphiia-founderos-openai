...
The generator rejects secret-like values, bearer tokens, Windows paths, and credential assignments. Its input is intentionally limited to Git metadata and the public progress projection. Do not add customer records, provider payloads, screenshots containing personal data, or raw session transcripts to the manifest.
...
```bash
/home/rlopez/projects/ralphiia-quoteops/.venv/bin/python -m pytest tests/test_build_evidence_report.py -q
/home/rlopez/projects/ralphiia-quoteops/.venv/bin/python -m pytest -q
git diff --check
```
